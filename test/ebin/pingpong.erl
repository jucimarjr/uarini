...
-export([bandwidth_calc/2, error_report/2, finalize/1,
	 generate_data/1, generate_data/2, new_/0, printResult/5,
	 run/2, run/3, time_microseg/0]).

new_() -> ObjectID = ooe:new([]), {pingpong, ObjectID}.
...
run(DataSize, R, OutFile) ->
    ...
    P1 = procpong:new(Data, Self, R),
    P2 = procpong:new(Data, Self, R),
